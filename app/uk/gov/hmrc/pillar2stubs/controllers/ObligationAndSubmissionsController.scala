...
class ObligationAndSubmissionsController @Inject() (cc: ControllerComponents, authFilter: AuthActionFilter)
  ...
  def getObligations: Action[ObligationsAndSubmissionsRequest] = (Action(parse.json[ObligationsAndSubmissionsRequest]) andThen authFilter) {
    implicit request =>
      request.headers.get("X-Pillar2-Id").get match {
        ...
        case _ =>
          if (request.body.accountingPeriodValid) Created(Json.toJson(ObligationsAndSubmissionsSuccessResponse()))
