...
@Singleton
class ORNSubmitController @Inject() (
  ...
  // This controller handles POST requests for ORN, which should now be ORN_CREATE
  def submitORN: Action[String] = (Action(parse.tolerantText) andThen authFilter).async { implicit request =>
    ...
    Try(Json.parse(request.body)) match {
      case Success(json) =>
        json.validate[UKTRSubmission] match { // Reusing UKTR model for simplicity in this stub implementation
              ...
              submission match {
                case d @ UKTRSubmissionData(_, _, _, _, _) =>
                  if (d.liabilities.liableEntities.isEmpty) {
                    ...
                    logger.info("Returning success response for liability request with submission type ORN_CREATE")
                    Future.successful(Created(successfulLiabilityResponse))
                  }
                case UKTRSubmissionNilReturn(_, _, _, _, _) =>
                  logger.info("Returning success response for Nil return request with submission type ORN_CREATE")
