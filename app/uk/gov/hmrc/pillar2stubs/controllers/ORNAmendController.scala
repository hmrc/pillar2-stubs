...
@Singleton
class ORNAmendController @Inject() (
  ...
  // This controller handles PUT requests for ORN, which should now be ORN_AMEND
  def amendORN: Action[String] = (Action(parse.tolerantText) andThen authFilter).async { implicit request =>
                  ...
                  submission match {
                    case d @ UKTRSubmissionData(_, _, _, _, _) =>
                      if (d.liabilities.liableEntities.isEmpty) {
                        ...
                        logger.info("Returning success response for liability amendment with submission type ORN_AMEND")
                        Future.successful(Ok(successfulLiabilityResponse))
                      }
                    case UKTRSubmissionNilReturn(_, _, _, _, _) =>
                      logger.info("Returning success response for Nil return amendment with submission type ORN_AMEND")
